C******************************************************************************|
C periodic.f, the fully-periodic-box solvers for zodiac.           VERSION 0.9
C
C These solvers were written by T. Bewley (spring 2001).
C******************************************************************************|
...
! Add a random phase
            U1(I,K,J)=cos(2*pi*(GY(J)+(RNUM1-0.5d0)*KICK/10.d0)/LY)
     &               *cos(2*pi*(GX(I)+(RNUM2-0.5d0)*KICK/10.d0)/LX)
     &               *SIN(2*pi*(GZ(K)+(RNUM3-0.5d0)*KICK/10.d0)/LZ)
            U2(I,K,J)=0.d0
            U3(I,K,J)=-cos(2*pi*(GY(J)+(RNUM1-0.5d0)*KICK/10.d0)/LY)
     &               *sin(2*pi*(GX(I)+(RNUM2-0.5d0)*KICK/10.d0)/LX)
     &               *COS(2*pi*(GZ(K)+(RNUM3-0.5d0)*KICK/10.d0)/LZ)
          END DO
        END DO
      END DO
...
C Any background stratification must be added to the governing equations
        ...
        DO J=0,NYM
          DO K=0,NZM
            DO I=0,NXM
!              CTH(I,K,J,N)=0.d0
!              TH(I,K,J,N)=0.1d0*cos(GY(J)+pi/4.d0)*cos(GX(I)+pi/4.d0)
!     &              *SIN(GZ(K)+pi/4.d0)
               TH(I,K,J,N)=0.d0
            END DO
          END DO
...
! Note that this routine uses CFi and CFTH for storage, so it should only be used between full
! R-K timesteps
...
! Output a 2d slice through the velocity field for animation in matlab
        open(79,file='movie_xz.txt',status='unknown',form='formatted')
        do i=0,NXM
        do k=0,NZM
          write(79,*) U1(I,K,NY/4)**2.+U3(I,K,NY/4)**2.+U2(I,K,NY/4)**2.
        end do
        end do

        open(80,file='movie_xy.txt',status='unknown',form='formatted')
        do i=0,NXM
        do j=0,NYM
...
! Chose which scalar is to be outputted
        open(75,file='movie.txt',status='unknown',form='formatted')
        do i=0,NXM
        do j=0,NYM
          write(75,*) TH(I,5,J,n)
        end do
